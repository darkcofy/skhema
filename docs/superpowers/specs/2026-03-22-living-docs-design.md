...
- **Lightweight client `manifest.yaml`** for metadata and section ordering
- **Simple regex markdown parser** — no dependencies, covers headings/paragraphs/bold/italic/code/links/lists/blockquotes
...
```
clients/<name>/
├── manifest.yaml              # Client metadata + section ordering
├── docs/
...
- "Open in new tab" link inside the modal for raw SVG access
- Vanilla CSS + JS, no dependencies

### Sticky Sidebar Navigation

- Left sidebar, `240px` wide
- Collapsible sections: C4, Sequence, ERD, Deployment (matching manifest order)
...
python scripts/docs.py --client demo --output novapay-v2.html # Custom filename
```
...
3. **Sections** — one per diagram type, in `manifest.yaml` order
...
- Accent color from `manifest.yaml` (default: `#D97706`)
...
- Missing `manifest.yaml`: use defaults (client name from folder, auto-detect sections from `rendered/` subdirectories)
...
- `clients/demo/deck.pdf` — PDF deck (re-rendered)
...
clients/demo/manifest.yaml     # New
