...
from skhema.render import resolve_includes
...
class TestRenderPlantUml:
    def test_renders_svg_via_subprocess(self):
        ...
        with patch("skhema.render.subprocess.run", return_value=mock_result) as mock_run:
    ...
    def test_missing_plantuml_binary(self):
        from skhema.render import render_plantuml
        with patch("skhema.render.subprocess.run", side_effect=FileNotFoundError()):
            try:
                render_plantuml("@startuml\nA -> B\n@enduml")
                assert False, "Should have raised"
            except RuntimeError as e:
